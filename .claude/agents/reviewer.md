...
10. Check orderable-content changes (`checklist_items`, `property_info`, `maintenance_items`, `favorite_places`):
    - Queries returning these rows must include `ORDER BY sort_order ASC`. Missing `ORDER BY sort_order` → `[nit]`. **Exception**: `maintenance_items` GET intentionally uses a multi-column status-priority sort (`CASE status WHEN 'in_progress' THEN 0 ...`, then `target_year NULLS LAST`, then `sort_order ASC`, then `created_at ASC`) — do **not** flag this as missing `ORDER BY sort_order ASC`; the `sort_order` column is still present in the ordering, just not the primary key.
    - New POST handlers that insert rows into orderable tables must compute the next `sort_order` as `COALESCE(MAX(sort_order), -1) + 1`. A hardcoded `sort_order = 0` or a missing sort_order assignment → `[nit]`. Note: `maintenance_items` and `favorite_places` do not have dedicated `/reorder` endpoints — do not flag their absence as missing.
    - When checking `favorite_places` inserts, valid `category` values are: `activity` (default), `culture`, `nature`, `outdoor`, `family`, `winter`, `dining`. An insert using a value outside this list → `[nit]` (these are not DB-enforced but are the established UI options).
